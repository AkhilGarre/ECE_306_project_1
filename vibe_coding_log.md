...
- Decided to host the repo under the existing personal GitHub account
  (`AkhilGarre`) rather than a separate school account — investigation
  showed the school Google identity (agarre@ncsu.edu) is linked as a
  sign-in method on the personal account, not a distinct account, so no
  real separate account exists without an unlinking step. Not required by
  the assignment, so using the personal account directly.
